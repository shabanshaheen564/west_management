...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}html,body{min-height:100%;margin:0}body{background:var(--page);color:var(--ink);font-family:var(--font);font-size:.9rem;overflow-x:hidden}a{color:var(--primary)}
        ...
        .topbar{position:fixed;top:0;inset-inline-start:var(--sidebar-w);inset-inline-end:0;width:calc(100vw - var(--sidebar-w));max-width:none;height:var(--top-h);background:rgba(255,255,255,.98);backdrop-filter:blur(14px);z-index:1050;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:1rem;padding:0 1.35rem;box-shadow:0 2px 14px rgba(20,45,60,.06);box-sizing:border-box;overflow:visible}
        html[dir="ltr"] .topbar{left:var(--sidebar-w);right:0;inset-inline-start:auto;inset-inline-end:auto}
        html[dir="rtl"] .topbar{left:0;right:var(--sidebar-w);inset-inline-start:auto;inset-inline-end:auto}
        .topbar-title{font-weight:800;font-size:1rem;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.topbar-actions{display:flex;align-items:center;justify-content:flex-end;gap:.55rem;flex:0 0 auto;min-width:max-content}.top-action{width:40px;height:40px;border:1px solid var(--line);background:#fff;border-radius:10px;display:grid;place-items:center;color:#60717d;position:relative;transition:.15s ease;padding:0;flex:0 0 auto}.top-action:hover{border-color:#b9d3dc;background:var(--primary-soft);color:var(--primary)}.top-action .avatar{width:30px;height:30px;flex-basis:30px;border-width:1px}.topbar .dropdown{flex:0 0 auto}.topbar .language-action{width:auto;min-width:40px;padding:0 .78rem;display:flex;align-items:center;justify-content:center;gap:.45rem;font-weight:700;font-size:.78rem;white-space:nowrap}.topbar .language-action i{font-size:1rem}.topbar .logout-action{color:var(--danger);border-color:#efd9dc}.topbar .logout-action:hover{background:#fff4f5;border-color:#e8b8be;color:#b9323d}.topbar .dropdown-menu{z-index:1060}.main-content{margin-inline-start:var(--sidebar-w);padding:1.35rem;min-height:100vh;padding-top:calc(var(--top-h) + 1.35rem);width:auto;min-width:0}html[dir="rtl"] .main-content{margin-inline-start:0;margin-inline-end:var(--sidebar-w)}
...
<body>
        ...
        <div class="topbar-actions">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><a class="dropdown-item {{ app()->getLocale()==='ar' ? 'active' : '' }}" href="{{ route('locale.switch','ar') }}">🇵🇸 العربية</a></li>
            ...
            <form action="{{ route('logout') }}" method="POST" class="m-0">
                @csrf
                <button class="top-action logout-action" type="submit" title="{{ __('Logout') }}" aria-label="{{ __('Logout') }}"><i class="fa-solid fa-right-from-bracket"></i></button>
            </form>
        </div>
    </header>
    ...
    <script>
        ...
        document.querySelectorAll('.sidebar a').forEach(a=>a.addEventListener('click',()=>{if(window.innerWidth<=900)closeSidebar()}));
